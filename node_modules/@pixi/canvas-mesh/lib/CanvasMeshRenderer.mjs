...
class CanvasMeshRenderer {
  ...
  render(mesh) {
    ...
    renderer.canvasContext.activeContext.globalAlpha = mesh.worldAlpha, renderer.canvasContext.setBlendMode(mesh.blendMode), renderer.canvasContext.setContextTransform(transform, mesh.roundPixels), mesh.drawMode !== DRAW_MODES.TRIANGLES ? this._renderTriangleMesh(mesh) : this._renderTriangles(mesh);
  }
  /**
  ...
  _renderDrawTriangle(mesh, index0, index1, index2) {
    ...
    mesh._cachedTexture && mesh._cachedTexture.baseTexture !== base && (mesh._cachedTint = 16777215, mesh._cachedTexture?.destroy(), mesh._cachedTexture = null, mesh._tintedCanvas = null), isTinted && mesh._cachedTint !== mesh.tintValue && (mesh._cachedTint = mesh.tintValue, mesh._cachedTexture = mesh._cachedTexture || new Texture(base), mesh._tintedCanvas = canvasUtils.getTintedCanvas(
